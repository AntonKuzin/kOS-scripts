...
until orbit:apoapsis >= targetAltitude
{
    ...
    set shipState["thrustVector"] to ship:facing * V(0, 0, -GetEnginesThrust(stagesData[currentStage]["engines"]:keys, shipState["radiusVector"]:mag - body:radius)).
    ...
    until predictedOrbit:apoapsis >= targetAltitude or shipState["radiusVector"]:mag - body:radius < 0
    {
        ...
        set shipState["thrustVector"] to -shipState["surfaceVelocityVector"]:normalized * GetEnginesThrust(stagesData[currentStage]["engines"]:keys, shipState["radiusVector"]:mag - body:radius).
